...
export default function VideoMetadata({ videoFile , setVideoFile  }: any) {
 ...
 //const {getEndPointUrl, uploadertoTus, progress, isUploaded, uploadedUrl, videoPlaybackId, isLoading } = usePinToLivepeer()
...
============================
  ...
  */
  ...
  useEffect(() => {
    if (status === "success"  && !isPostUploaded) {
      handlePostToBack();
    }
  }, [status]);
...
<div className="flex-1  h-full  px-3">
        ...
        <div className="my-4 ">
          ...
          <h1 className="text-2xl">{progressFormatted}</h1>
