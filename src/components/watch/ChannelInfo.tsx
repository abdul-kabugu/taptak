...
export default function ChannelInfo({channel}: channelProps) {
  ...
  const {data, isLoading, error}  = useGetUserInfo(channel)

  const router = useRouter();

    console.log("the channel  stats", data)
  return (
    <div className='flex justify-between items-center my-3'>
        ...
        <div>
             <h1 className='text-sm font-semibold md:font-normal  hover:text-rose-600 md:text-lg'>{data?.username}</h1>
              <p className='text-xs md:text-sm font-mono'>10 Followers</p>
        </div>
        <h1 className='text hidden'>{ truncateText( data?.address, 6)}</h1>
        </div>
