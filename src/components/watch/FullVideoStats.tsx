...
export default function FullVideoStats({stats, createdAt, videId, tips, likes, isLiked, note} : statsProps) {
  ...
  const duration = moment.duration(diffInHours, "hours");
    ...
    const [isShowTradeModal, setisShowTradeModal] = useState(false)
    const {handleLike, isPostLiked, likeStatus, postLikesCount, postLikes}  = useReactions()
  ...
  /*  const  contract = new Contract(contractAddress, ABI, signer)
    ...
    console.log("Character created  here is the tx id", transaction);
        ...
        useEffect(() => {
          ...
          console.log("the like  status", likeStatus)
          console.log("the like  count", postLikesCount)


            const  handleInvokeLike  =  ()  =>  {
              if(! likeStatus?.isLiked){
                handleLike(videId, userProfile?.id)
              }else{
                console.log("You've liked")
              }
            }
...
<p className='xs:text-xs'>{duration.humanize().replace("a", "")} ago</p>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
   ...
   <p className='text-xs'>{stats?.viewCount || "0"} views</p>
         ...
         <div className={`flex gap-6 items-center md:justify-center  mt-4 md:my-0 `}>
              ...
              <div className={`flex items-center gap-2 ${  likeStatus?.isLiked && "text-green-500"} cursor-pointer `} onClick={()  => handleInvokeLike()}>
  ...
  <p className=' text-sm md:font-semibold'>{postLikesCount?.totalLikes || "0"}</p>
              </div>
...
<p className='text-sm md:font-semibold'>Support</p>
          ...
          <div className='flex items-center gap-2 hover:text-text-primary cursor-pointer' onClick={() => setisShowTradeModal(!isShowTradeModal)}>
...
<p className='text-sm md:font-semibold'>Buy / Sell</p>
        ...
        <TooltipProvider>
           <Tooltip>
            <TooltipTrigger>
            <div className=' items-center gap-2 text-green-500 cursor-pointer  flex' >
       <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
...
<p className='text-sm font-semibold'>Share</p>
  ...
  <Modal isOpen={isShowTipModal} closeModal={() => setisShowTipModal(!isShowTipModal)}
   withCloseButton
   panelClassName='bg-gray-200  dark:bg-gray-800'
  >
  ...
  <Modal isOpen={isShowTradeModal} closeModal={() => setisShowTradeModal(!isShowTradeModal)}
   withCloseButton
   panelClassName='bg-gray-200  dark:bg-gray-800'
  >
